Shader "World Political Map/Unlit Earth Scenic City Lights" {
	...
	Subshader {
		...
		Pass {
			...
			float2 disp = -input.viewDir.xy * _CloudElevation;
				...
				// Add city lights
				float4 cityLights = SAMPLE_TEXTURE2D(_CityLights, sampler_CityLights, input.uv);
				float cityLightsMask = (1.0 - LdotS) * cityLights.a * _CityLightsBrightness;
				rgb.rgb += cityLights.rgb * cityLightsMask;
	...
	// Add back the built-in pipeline subshader
	Subshader {
