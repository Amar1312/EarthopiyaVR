...
Version 19.6
- API: added "DrawCountriesOutline" method
- Updated Demo 18 Continents to demonstrate the DrawCountriesOutline method

Version 19.5
- Added "Draw Outline" option to province highlight section
- API: added ResetCamera method (resets any yaw/pitch and optionally zoom level)
- API: added optional parameters to ToggleProvinceSurface to support adding outlines
- API change: passing 0 as destination zoom level to FlyToLocation will not zoom to level 0 instead of keeping the current zoom level
- Tilt on zoom: the option now works always when camera zooms in and not only when using the mouse wheel
- [Fix] Fix for enclosing rectangle calculation for world-edge crossing countries

Version 19.4
- Impoved rotation accuracy by using a high precision transform when "Precise Rotation" option is enabled
- Updated tile map server list

Version 19.3
- Colored/textured surfaces are now recreated automatically when redrawing the map
- [Fix] Fixed CountryTransferCell issue on new countries
