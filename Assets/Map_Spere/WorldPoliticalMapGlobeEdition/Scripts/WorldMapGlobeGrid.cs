...
namespace WPM {
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        public void FlyToCell(int cellIndex) {
            if (cellIndex < 0 || cellIndex >= cells.Length)
                return;
            FlyToLocation(cells[cellIndex].sphereCenter, _navigationTime, -1f, _navigationBounceIntensity);
        }
        ...
        public void FlyToCell(int cellIndex, float duration) {
            if (cellIndex < 0 || cellIndex >= cells.Length)
                return;
            FlyToLocation(cells[cellIndex].sphereCenter, duration, -1f, _navigationBounceIntensity);
        }
