...
namespace WPM {
    ...
    public delegate void ProvinceClickEvent(int provinceIndex, int regionIndex, int buttonIndex);
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        public event ProvinceClickEvent OnProvincePointerDown;
        public event ProvinceClickEvent OnProvincePointerUp;
        public event ProvinceClickEvent OnProvinceClick;
        ...
        [SerializeField]
        float
            _provinceHighlightFadeDuration;

        /// <summary>
        /// Duration of the fade
        /// </summary>
        public float provinceHighlightFadeDuration {
            get {
                return _provinceHighlightFadeDuration;
            }
            set {
                if (_provinceHighlightFadeDuration != value) {
                    _provinceHighlightFadeDuration = Mathf.Max(0, value);
                    isDirty = true;
                }
            }
        }
        ...
        [SerializeField]
        bool
            _showProvinceOutline = true;

        /// <summary>
        /// Toggle province outline visibility by default when coloring provinces.
        /// </summary>
        public bool showProvinceOutline {
            get {
                return _showProvinceOutline;
            }
            set {
                if (value != _showProvinceOutline) {
                    _showProvinceOutline = value;
                    isDirty = true;
                }
            }
        }
        ...
        [SerializeField]
        Color
            _provinceOutlineColor = Color.black;

        /// <summary>
        /// Default outline color for provinces when drawing outlines.
        /// </summary>
        public Color provinceOutlineColor {
            get { return _provinceOutlineColor; }
            set {
                if (value != _provinceOutlineColor) {
                    _provinceOutlineColor = value;
                    isDirty = true;
                }
            }
        }
        ...
        public bool ToggleProvinceSurface(int provinceIndex, bool visible, Color color) {
            ...
            for (int r = 0; r < regionCount; r++) {
                ToggleProvinceRegionSurface(provinceIndex, r, visible, color, null, Misc.Vector2one, Misc.Vector2zero, 0, _showProvinceOutline, _provinceOutlineColor);
            }
            return true;
        }

        /// <summary>
        /// Colorize all regions of specified province/state by index and optionally draw/remove outlines.
        /// </summary>
        public void ToggleProvinceSurface(int provinceIndex, bool visible, Color color, bool drawOutline, Color outlineColor) {
            if (!visible) {
                ToggleProvinceOutline(provinceIndex, false);
                HideProvinceSurfaces(provinceIndex);
                return;
            }
            if (provinceIndex < 0 || provinceIndex >= provinces.Length)
            ...
            int regionCount = provinces[provinceIndex].regions.Count;
            for (int r = 0; r < regionCount; r++) {
                ToggleProvinceRegionSurface(provinceIndex, r, true, color, null, Misc.Vector2one, Misc.Vector2zero, 0, drawOutline, outlineColor);
            }
        }
        ...
        public void ToggleProvinceMainRegionSurface(int provinceIndex, bool visible, Color color) {
            ...
            ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, color, null, Misc.Vector2one, Misc.Vector2zero, 0, _showProvinceOutline, _provinceOutlineColor);
        }
        ...
        public void ToggleProvinceMainRegionSurface(int provinceIndex, bool visible, Color color, Texture2D texture) {
            if (provinceIndex < 0 || provinceIndex >= provinces.Length)
                return;
            int regionIndex = provinces[provinceIndex].mainRegionIndex;
            ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, color, texture, Misc.Vector2one, Misc.Vector2zero, 0, _showProvinceOutline, _provinceOutlineColor);
        }
        ...
        public void ToggleProvinceMainRegionSurface(int provinceIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
            if (provinceIndex < 0 || provinceIndex >= provinces.Length)
                return;
            int regionIndex = provinces[provinceIndex].mainRegionIndex;
            ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, color, texture, textureScale, textureOffset, textureRotation, _showProvinceOutline, _provinceOutlineColor);
        }
        ...
        public void ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, Color color) {
            ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, color, null, Vector2.one, Vector2.zero, 0, _showProvinceOutline, _provinceOutlineColor);
        }
        ...
        public GameObject ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool temporary) {
            return ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, color, texture, textureScale, textureOffset, textureRotation, _showProvinceOutline, _provinceOutlineColor);
        }

        /// <summary>
        /// Colorize or texture a region of specified province/state by index in the provinces collection and optionally draw outline.
        /// </summary>
        public GameObject ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool drawOutline, Color outlineColor) {
            ...
            if (surf != null) {
                ...
                surf = GenerateProvinceRegionSurface(provinceIndex, regionIndex, surfMaterial, textureScale, textureOffset, textureRotation, false);
            }
            // draw/remove outline
            if (drawOutline) {
                ToggleRegionOutline(region, true, outlineColor);
            } else {
        ...
        /// <summary>
        /// Adds a province outline for the main region
        /// </summary>
        public GameObject DrawProvinceOutline(string countryName, string provinceName, Color color) {
            int provinceIndex = GetProvinceIndex(countryName, provinceName);
            return DrawProvinceOutline(provinceIndex, color);
        }

        /// <summary>
        /// Adds a province outline for the main region
        /// </summary>
        public GameObject DrawProvinceOutline(int provinceIndex, Color color) {
            ...
            GameObject surf = region.surfaceGameObject;
            if (surf == null) {
                // create a temporary holder for outline if no surface yet
                surf = new GameObject(SURFACE_GAMEOBJECT);
            ...
            return DrawRegionOutline(region, surf, false, color);
        }

        /// <summary>
        /// Toggles on/off a province outline (all regions)
        /// </summary>
        public void ToggleProvinceOutline(string countryName, string provinceName, bool visible, Color color = default(Color)) {
            int provinceIndex = GetProvinceIndex(countryName, provinceName);
            ToggleProvinceOutline(provinceIndex, visible, color);
        }

        /// <summary>
        /// Toggles on/off a province outline (all regions)
        /// </summary>
        public void ToggleProvinceOutline(int provinceIndex, bool visible, Color color = default(Color)) {
            if (!ValidProvinceIndex(provinceIndex)) return;
            Province province = provinces[provinceIndex];
            if (province.regions == null) ReadProvincePackedString(province);
            int regionsCount = province.regions.Count;
            for (int k = 0; k < regionsCount; k++) {
                ToggleRegionOutline(province.regions[k], visible, color == default(Color) ? _provinceOutlineColor : color);
            }
        }

        /// <summary>
        /// Toggles on/off a province region outline
        /// </summary>
        public GameObject ToggleProvinceRegionOutline(int provinceIndex, int regionIndex, bool visible, Color color = default(Color)) {
            if (!ValidProvinceRegionIndex(provinceIndex, regionIndex)) return null;
            Region region = provinces[provinceIndex].regions[regionIndex];
            return ToggleRegionOutline(region, visible, color == default(Color) ? _provinceOutlineColor : color);
        }
        ...
        public int ProvinceToCountry(Province province, string newCountryName, bool redraw = true) {
            ...
            if (province == null || province.countryIndex < 0 || province.regions == null || string.IsNullOrEmpty(newCountryName))
                return -1;
            ...
            Country newCountry = new Country(newCountryName, continent);
        ...
        public int ProvincesToCountry(List<Province> provinces, string newCountryName, bool redraw = true) {
            ...
            // Add new country
            string continent = GetCountry(provinces[0].countryIndex).continent;
            Country newCountry = new Country(newCountryName, continent);
