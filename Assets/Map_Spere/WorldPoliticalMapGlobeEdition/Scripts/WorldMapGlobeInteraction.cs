...
namespace WPM {
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        /// <summary>
        /// Resets the camera to default position and zoom level.
        /// This properly centers the globe and removes any pivot offset from off-center zooming.
        /// </summary>
        /// <param name="zoomLevel">Optional zoom level (0-1). If not specified (value < 0), preserves current zoom level.</param>
        /// <param name="duration">Optional animation duration. If 0, resets immediately.</param>
        public void ResetCamera(float zoomLevel = -1f, float duration = 0f) {
            // Reset zoom level - preserve current zoom if not specified (zoomLevel < 0)
            if (zoomLevel >= 0) {
                if (duration > 0) {
            ...
            // Reset camera pivot to center on the globe (fixes off-center zoom artifacts)
            if (_zoomMode == ZOOM_MODE.CAMERA_MOVES) {
                Vector3 dir = (pivotTransform.position - transform.position).normalized;
                float distance = GetCameraDistance();
                pivotTransform.position = transform.position + dir * distance;
                
                // Reset pivot rotation to look at the globe center
                pivotTransform.LookAt(transform.position, transform.up);
            }
            
            // Reset globe rotation and straighten
            StraightenGlobe(duration, true);
        ...
        public CallbackHandler FlyToLocation(Vector3 destination) {
            return FlyToLocation(destination, _navigationTime, -1f, _navigationBounceIntensity);
        }
        ...
        /// </summary>
        public CallbackHandler FlyToLocation(Vector2 latlon, float duration, float destinationZoomLevel = -1f) {
        ...
        /// Navigates to target latitude and longitude using given duration.
        /// </summary>
        /// <param name="destinationZoomLevel">Destination zoom level. A value of -1 will keep current zoom level.</param>
        public CallbackHandler FlyToLocation(float latitude, float longitude, float duration, float destinationZoomLevel = -1f) {
        ...
        public CallbackHandler FlyToLocation(Vector3 destination, float duration, float destinationZoomLevel = -1f, float bounceIntensity = 0, float cameraDistance = 0) {
            ...
            if (destinationZoomLevel >= 0 || bounceIntensity > 0) {
                flyToCameraDistanceEnd = destinationZoomLevel >= 0 ? GetZoomLevelDistance(Mathf.Clamp01(destinationZoomLevel)) : flyToCameraDistanceStart;
