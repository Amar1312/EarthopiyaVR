...
namespace WPM
{
    ...
    public partial class WorldMapGlobe : MonoBehaviour
    {
        ...
        Vector3 zoomDir;
        public IHighPrecisionTransform globeTransform;
        ...
        void OnEnable()
        {
            ...
            if (_tilePreciseRotation && Application.isPlaying)
            {
                globeTransform = new HighPrecisionTransform(transform);
            }
            else
            {
                globeTransform = new NormalPrecisionTransform(transform);
            }
        ...
        //public string targetLayerName = "Map";
        ...
        //            if (clickedObject.layer == LayerMask.NameToLayer(targetLayerName))
        //            {
        //                Debug.Log("Clicked UI Image IS on Map layer!");
        //                //OnMapImageClicked(clickedObject);
        //                return true;
        //            }
        //            else
        //            {
        //                Debug.Log("Clicked UI Image is NOT on Map layer.");
        //                //OnOtherImageClicked(clickedObject);
        //                return false;
        //            }
        //            break; // stop after first object hit
        //        }
        //    }
        //}
        private bool Check;
        ...
        void CheckMouseOver()
        {
            ...
            //        {
            //            // Check object's layer
            //            if (clickedObject.layer == LayerMask.NameToLayer("Map"))
            //            {
            //                Debug.Log("Clicked UI Image IS on Map layer!");
            //                //OnMapImageClicked(clickedObject);
            //                Check = true;
            ...
            ////sdasda
            //Vector3 Point = ray(out ColliderHit);
        ...
        void PerformAutoRotation()
        {
            ...
            else
            {
                ...
                if (!constraintPositionEnabled)
                {
                    ...
                    {
                        globeTransform.Rotate(Misc.Vector3up, -_autoRotationSpeed * Time.deltaTime * 60f);
                    }
        ...
        Quaternion GetCameraStraightLookRotation()
        {
            ...
            if (Mathf.Abs(Vector3.Dot(transform.up, camVec.normalized)) > 0.96f)
            {   // avoid going crazy around poles
                    ...
                    else if (!lockPan && mouseStartedDragging && (leftMouseButtonPressed || (_rightButtonDragBehaviour == DRAG_BEHAVIOUR.Drag && rightMouseButtonPressed) || touchPadTouchStays) && input.touchCount < 2)
                    {
                                    ...
                                    if (input.mousePosition != mouseDragStart)
                                    {
                                        ...
                                        if (dragAngle != 0)
                                        {
                                            ...
                                            if (_navigationMode == NAVIGATION_MODE.EARTH_ROTATES)
                                            {
                                                ...
                                                globeTransform.Rotate(dragAxis, dragAngle);
                                            }
                                            else
                                            {
                        ...
                        else if (!gestureAborted)
                        {
                            ...
                            if (dragDirection.x != 0 || dragDirection.y != 0)
                            {
                                ...
                                if (_navigationMode == NAVIGATION_MODE.EARTH_ROTATES)
                                {
                                    globeTransform.Rotate(cam.transform.up, dragDirection.x, Space.World);
                                    Vector3 axisY = Vector3.Cross(transform.position - cam.transform.position, cam.transform.up);
                                    globeTransform.Rotate(axisY, dragDirection.y, Space.World);
                                }
                                else
                                {
                                    if (_rotationAxisAllowed == ROTATION_AXIS_ALLOWED.X_AXIS_ONLY)
        ...
        protected virtual void CheckUserInteractionInvertedMode()
        {
                ...
                if (_allowUserRotation)
                {
                    ...
                    else if (mouseStartedDragging && leftMouseButtonPressed && input.touchCount < 2)
                    {
                            ...
                            if (CheckDragThreshold(mouseDragStart, input.mousePosition, _mouseDragThreshold))
                            {
                                ...
                                if (dragAngle != 0)
                                {
                                    ...
                                    if (_navigationMode == NAVIGATION_MODE.EARTH_ROTATES)
                                    {
                                        ...
                                        globeTransform.Rotate(dragAxis, dragAngle, Space.World);
                                    }
                                    else
                                    {
                                        Vector3 v1 = transform.TransformPoint(mouseDragStartCursorLocation) - transform.position;
                        ...
                        else
                        {
                            ...
                            if (dragDirection.x != 0 && dragDirection.y != 0)
                            {
                                ...
                                if (_navigationMode == NAVIGATION_MODE.EARTH_ROTATES)
                                {
                                    globeTransform.Rotate(Misc.Vector3up, dragDirection.x, Space.World);
                    ...
                    if (rightMouseButtonPressed && input.touchCount < 2 && !flyToActive)
                    {
                        ...
                        else
                        {
                            ...
                            float rotAngle = _rightClickRotatingClockwise || input.GetKey(KeyCode.LeftShift) || input.GetKey(KeyCode.RightShift) ? -2f : 2f;
                            globeTransform.Rotate(axis, rotAngle * Time.deltaTime * 60f, Space.World);
                        }
                    }
                }
            }
            ...
            if (dragDampingStart > 0)
            {
                ...
                if (t >= 0 && t <= 1f)
                {
                    if (_navigationMode == NAVIGATION_MODE.EARTH_ROTATES)
                    {
                        globeTransform.Rotate(Misc.Vector3up, dragDirection.x * t, Space.World);
                        Vector3 axisY = Vector3.Cross(transform.position - cam.transform.position, Misc.Vector3up);
                        globeTransform.Rotate(axisY, dragDirection.y * t, Space.World);
                    }
                    else
                    {
            ...
            if (constraintPositionEnabled && mouseStartedDragging)
            {
                ...
                if (mouseIsOver)
                {
                    ...
                    if (Mathf.Abs(angleDiff) > constraintPositionAngle + 0.0001f)
                    {
                        ...
                        {
                            axis.z = 0;
                            globeTransform.Rotate(axis, -angleDiff, Space.World);
                        }
                        dragDampingStart = 0;
        ...
        bool GetRay(out Ray ray)
        {
            ...
            else
            {
                ...
                // Debug.DrawRay(mousePos, cam.transform.forward);
            }
#endif
        ...
        public void RegionSanitize(List<Region> regions, bool forceSanitize)
        {
            ...
            for (int k = 0; k < regionCount; k++)
            {
                ...
                { // remove invalid regions (<3 points)
                    regions.RemoveAt(k);
                    k--;
                    regionCount--;
                }
        ...
        bool RegionMagnet(Region region, Region neighbourRegion)
        {
            ...
            { // maximum iterations = pointCount ; also avoid any potential (rare) infinite loop (good practice)
