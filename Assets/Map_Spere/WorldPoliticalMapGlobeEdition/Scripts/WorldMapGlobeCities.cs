...
namespace WPM {
    ...
    public delegate void CityClickEvent(int cityIndex, int buttonIndex);
    ...
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        public event CityClickEvent OnCityPointerDown;
        public event CityClickEvent OnCityPointerUp;
        public event CityClickEvent OnCityClick;
        ...
        [SerializeField]
        bool
            _enableCityHighlight = true;

        /// <summary>
        /// Enable/disable city highlight when mouse is over.
        /// </summary>
        public bool enableCityHighlight {
            get {
                return _enableCityHighlight;
            }
            set {
                if (_enableCityHighlight != value) {
                    _enableCityHighlight = value;
                    isDirty = true;
                }
            }
        }
        ...
        [SerializeField]
        Color
            _citiesHighlightColor = Color.gray;

        /// <summary>
        /// Color for city highlight.
        /// </summary>
        public Color citiesHighlightColor {
            get {
                return _citiesHighlightColor;
            }
            set {
                if (value != _citiesHighlightColor) {
                    _citiesHighlightColor = value;
                    isDirty = true;
                }
            }
        }
        ...
        public CallbackHandler FlyToCity(int cityIndex, float duration) {
            if (cityIndex < 0 || cityIndex >= cities.Count)
                return CallbackHandler.Null;
            return FlyToLocation(cities[cityIndex].localPosition, duration, -1f, _navigationBounceIntensity);
        }
        ...
        public CallbackHandler FlyToCity(City city, float duration) {
            return FlyToLocation(city.localPosition, duration, -1f, _navigationBounceIntensity);
        }
        ...
        public void HideCityHighlights() {
            if (citiesLayer == null || cities == null || cityProps == null)
                return;
            ...
            for (int k = 0; k < citiesCount; k++) {
                ...
                switch(city.cityClass) {
                    case CITY_CLASS.COUNTRY_CAPITAL:
                        r.sharedMaterial = citiesCountryCapitalMat;

                        break;
                    case CITY_CLASS.REGION_CAPITAL:
                        r.sharedMaterial = citiesRegionCapitalMat;
                        break;

                    default:
                        r.sharedMaterial = citiesNormalMat;
                        break;
        ...
        public void ToggleCityHighlight(int cityIndex, Color color, bool highlighted) {
            ...
            if (citiesLayer == null)
                return;
            Renderer cityRenderer = cities[cityIndex].renderer;
            if (cityRenderer == null)
                return;
            if (highlighted) {
                SetCityColor(cityIndex, color);
            } else {
                SetCityColor(cityIndex, GetCityColorByClass(cityIndex));
            }
        }
        ...
        public void BlinkCity(int cityIndex, Color color1, Color color2, float duration, float blinkingSpeed) {
            ...
            if (cityRenderer == null)
                return;
            CityBlinker sb = cityRenderer.gameObject.AddComponent<CityBlinker>();
            sb.blinkMaterial = cityRenderer.sharedMaterial;
            sb.color1 = color1;
            sb.color2 = color2;
