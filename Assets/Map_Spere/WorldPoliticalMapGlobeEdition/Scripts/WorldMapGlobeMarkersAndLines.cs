...
namespace WPM
{
    ...
    public partial class WorldMapGlobe : MonoBehaviour
    {
        ...
        /// <summary>
        /// Toggle tropic lines visibility.
        /// </summary>
        public bool showTropicLines
        {
            get
            {
                return _showTropicLines;
            }
            set
            {
                if (value != _showTropicLines)
                {
                    _showTropicLines = value;
                    isDirty = true;

                    if (tropicLayer != null)
                    {
                        tropicLayer.SetActive(_showTropicLines);
                    }
                    else
                    {
                        DrawTropicLines();
        ...
        [SerializeField]
        Color
            _tropicLinesColor = new Color(0.16f, 0.63f, 0.498f);

        /// <summary>
        /// Color for tropic lines.
        /// </summary>
        public Color tropicLinesColor
        {
            get
            {
                return _tropicLinesColor;
            }
            set
            {
                if (value != _tropicLinesColor)
                {
                    _tropicLinesColor = value;
                    isDirty = true;

                    if (tropicMatOverlay != null && _tropicLinesColor != tropicMatOverlay.color)
                    {
                        tropicMatOverlay.color = _tropicLinesColor;
                    }
                    if (tropicMatMasked != null && _tropicLinesColor != tropicMatMasked.color)
                    {
